	...
	; -M.ca/dist/PRO_Comparison/production/PIC18F4620.production.map \
	; -DXPRJ_PRO_Comparison=PRO_Comparison -L--defsym=__MPLAB_BUILD=1 \
	; --dfp=D:/MPLAP/packs/Microchip/PIC18Fxxxx_DFP/1.3.36/xc8 \
	; --opt=+asmfile --addrqual=ignore -P --warn=-3 --asmlist --std=c99 \
	; --output=+elf:multilocs --stack=compiled:auto:auto:auto \
	; --summary=+xml \
	; --summarydir=.ca/dist/PRO_Comparison/production/memoryfile.xml \
	; -oPIC18F4620.production.elf \
	; --objdir=.ca/dist/PRO_Comparison/production \
	; --outdir=.ca/dist/PRO_Comparison/production \
	; .ca/build/PRO_Comparison/production/ECU_layer/led/ECU_led.p1 \
	; .ca/build/PRO_Comparison/production/MCAL_Layer/DeviceConfig/DeviceConfig.p1 \
	; .ca/build/PRO_Comparison/production/MCAL_Layer/GPIO/HAL_GPIO.p1 \
	; .ca/build/PRO_Comparison/production/app.p1 -L--fixupoverflow=error \
	; --callgraph=none --errformat=%f:%l:%c: error: (%n) %s \
	; --warnformat=%f:%l:%c: warning: (%n) %s \
	; --msgformat=%f:%l:%c: advisory: (%n) %s \
	; --summarycmp=C:\Users\yosse\AppData\Local\Temp\xcAsl6g.
	;
